	...
	<div style="width: 100%; padding-top: 1%; text-align:center; margin:0 auto">
		<div class="nanum"style="margin:0 auto;width:70%;padding-top:5%;margin-bottom:10%;">
			...
			<form onsubmit="return BM()" action="/boardDelete.do"style="display: inline;">
				...
				<button style="font-family:sans-serif;margin-left:2px;" class="btn btn-success" onclick="location.href='userBoard.do?Pno=1'">목록</button>		
			</div>
		</div>
		</div>
		<div style="width: 100%; padding-top: 1%;margin-bottom: 5%;border-top:1px dashed lightgray">
		<div style="width:70%;margin:0 auto;text-align: center;">
			<div class="boardTitle nanum" style="font-size:3rem">작성게시글 확인</div>
			<div style="color: rgba(0,0,0,0.3); font-size:0.8rem">회원님께서 작성하신 글입니다.</div>
		...
		<div class="listWrapper">
			...
			<%
				if(paging.isPrev()==true) { %>
					<a class="pNumStart" href="/userBoardDetail.do?no=<%=Seq %>&Pno=<%=paging.getStartPage()-10%>">이전</a>
			<%}	%>
			...
			<%} else {%>
					<a class='pNum' href="/userBoardDetail.do?no=<%=Seq %>&Pno=<%=a%>"><%=a%></a>
			<%}}%>
			<%
				if(paging.isNext()==true) { %>
					<a class="pNumEnd" href="/userBoardDetail.do?no=<%=Seq %>&Pno=<%=paging.getEndPage()+1%>">다음</a>
			<%}	%>
			</div>
		</div>
	</div>
