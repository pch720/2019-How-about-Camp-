	...
	<div style="width: 100%; padding-top: 1%;margin-bottom: 5%;">
		...
		<div class="listWrapper">
			...
			<div class="listTag">
				...
				<div class="listTitle" onclick="location.href='userBoardDetail.do?no=<%=bList.get(i).getBoardSeq() %>&Pno=1'"><a href="#"><%=bList.get(i).getBoardTitle().replace(">","&gt;").replace("<","&lt;").replace("'","&#39;").replace("script", "")  %>
			...
			<%
				if(paging.isPrev()==true) { %>
					<a class="pNumStart" href="/userBoard.do?Pno=<%=paging.getStartPage()-10%>">이전</a>
			<%}	%>
			...
			<%} else {%>
					<a class='pNum' href="/userBoard.do?Pno=<%=a%>"><%=a%></a>
			<%}}%>
			<%
				if(paging.isNext()==true) { %>
					<a class="pNumEnd" href="/userBoard.do?Pno=<%=paging.getEndPage()+1%>">다음</a>
			<%}	%>
			</div>
		</div>
	</div>
